...
import { mockPullRequest } from '@/lib/mockData'
          ...
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="bg-primary/10 flex items-center justify-center rounded-xl w-full max-w-[400px] aspect-video border border-primary/20 relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col items-center gap-3">
                ...
                <span className="text-xs font-mono text-slate-400">PR #{pr.github_pr_number}</span>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3 justify-center">
              ...
              <button
                onClick={() => navigator.clipboard.writeText(pr.github_pr_url)}
        ...
        <section className="px-4 py-4">
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4 space-y-1">
            {[
              ...
              { label: 'Commit', value: `"${pr.commit_message}"`, icon: undefined, isItalic: true },
              ...
              <div key={i} className={`flex justify-between items-center gap-x-6 py-2 ${
                ...
                <div className="flex items-center gap-2">
                  ...
                  {row.isCode ? (
                    ...
                    <p className={`text-sm text-right ${row.isBold ? 'text-white font-bold' : row.isItalic ? 'text-white italic' : 'text-white font-semibold'}`}>
                      {row.value}
                    </p>
                  )}
                </div>
          ...
          <div className="flex flex-col items-start justify-between gap-6 rounded-xl border border-primary/30 bg-primary/10 p-6">
            <div className="flex flex-col gap-2">
              ...
              <p className="text-slate-300 text-sm font-normal leading-relaxed">
                This PR fixes a SQL injection vulnerability in the session authentication module by replacing raw SQL string interpolation with Prisma ORM type-safe queries. The fix prevents unauthorized data access through crafted session IDs. Key improvements include parameterized queries, proper null handling, and updated type definitions for the User model.
              </p>
            </div>
          </div>
          <div className="mt-4 flex justify-end">
            <button className="text-xs font-bold leading-normal tracking-wide flex items-center gap-1 text-slate-400 hover:text-primary transition-colors">
